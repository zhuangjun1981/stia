...
def align_multiple_files_iterate_anchor_multi_thread(input_folder,
    ...
    :param anchor_frame_ind: non-negative int, frame index for first correction iteration for each file
    ...
    print ('finding files to correct ...')
    ...
    print ('correction output will be saved in {}.'.format(os.path.abspath(output_folder)))

    print ('\naligning single chunks:')
    params_lst = [(f, anchor_frame_ind, iteration_chunk, max_offset, align_func, fill_value,
    ...
    for i, file_path in enumerate(file_paths):
        ...
        offsets_dset = offsets_f.create_dataset('file' + bas.int2str(i, 4),
                                                data=curr_chunk_offsets + curr_global_offset)
        offsets_dset.attrs['format'] = ['height', 'width']
        offsets_dset.attrs['path'] = file_path
    offsets_f.close()
    print ('chunks aligned offsets and projection images saved.')

    if avi_downsample_rate is not None:
        apply_correction_offsets_multi_thread(offsets_path=os.path.join(output_folder, "correction_offsets.hdf5"),
                                              path_pairs=zip(f_paths, file_paths),
...
def apply_correction_offsets_multi_thread(offsets_path,
                                          path_pairs,
                                          process_num=1,
                                          fill_value=0.,
                                          output_folder=None,
    ...
    if avi_downsample_rate is not None:
        ...
        mov_down = np.concatenate(mov_downs, axis=0)
        ...
        if cv2.__version__[0:3] == '3.1':
            ...
            out = cv2.VideoWriter(os.path.join(output_folder, mov_name), fourcc, 30,
                                  (mov_down.shape[2], mov_down.shape[1]), isColor=False)
